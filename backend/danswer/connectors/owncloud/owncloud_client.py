...
class OwnCloudClient:
    ...
    def _dispatch_authenticated_request(
        ...
        return requests.request(
            method=method, url=url, auth=(self.username, self.password), **kwargs
        )
    ...
    def get_all_files(self) -> list[OwnCloudFileObject | None]:
        response = self._dispatch_authenticated_request(
            method="PROPFIND",
            url=f"remote.php/dav/files/{self.username}",
            headers={"Depth": "100000000"},
        )
    ...
    def get_file_content(self, file_path, stream=True):
        response = self._dispatch_authenticated_request(
            method="GET",
            url=f"remote.php/dav/files/{self.username}/{file_path}",
            stream=stream,
        )
